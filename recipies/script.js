...
function search() {
    cardQuery = searchInput.value
    
    let filteredCards = recipes.filter(function(card){
        console.log(cardQuery)
        return (
        card.name.toLowerCase().includes(cardQuery.toLowerCase()) ||
        card.description.toLowerCase().includes(cardQuery.toLowerCase()) ||
        card.tags.find(tag => tag.toLowerCase().includes(cardQuery.toLowerCase()))
        )
    })

    let sortedCards = filteredCards.sort(card => card.name.toLowerCase())

    cardContainer.innerHTML = ''
    sortedCards.forEach(recipe => {
        renderCard(recipe)
    });
}

function tagsTemplate(tags){
    return tags.map((tag) => `<p class="mealGenre">${tag}<p>`).join('')
} 
...
function cardTemplate(card) { 
    return `<div class="recipe">
            <div class="imageContainer">
                ...
                <div class="mealGenreContainer">${tagsTemplate(card.tags)}</div>
            </div>
            <div class="recipeInfo">
                <h2>${card.name}</h2>
                ${ratingTemplate(card.rating)}
                <p class="recipeDescription">${card.description}</p>          
            </div>
        </div>`
}

function renderCard(card) {
    let html = cardTemplate(card)
